...
export const AlluserswithdrawRequest = () => async (dispatch) => {
  ...
  export const AllusersdepositRequest = () => async (dispatch) => {
    
    try {
      dispatch({ type: ALL_DEPOSIT_REQ_BY_ALL_USERS_REQUEST });
      ...
      const { data } = await axios.get(
        `${backedurl}/api/v1/admin/allrequsetfordeposit `,

        config
      );
      

      dispatch({ type: ALL_DEPOSIT_REQ_BY_ALL_USERS_SUCCESS, payload: data });
    } catch (error) {
      
      dispatch({
        type: ALL_DEPOSIT_REQ_BY_ALL_USERS_FAIL,
        payload: error.response
          ? error.response.data.error
          : "Unauthorized: please try again after some time",
      });
...
export const claimUserreward = (dataform) => async (dispatch) => {
  try {
    dispatch({ type: CLAIMED_REFER_REWARD_REQUEST });
    ...
    const { data } = await axios.put(
      `${backedurl}/api/v1/claimbonus`,
      dataform,
      config
    );

    dispatch({ type: CLAIMED_REFER_REWARD_SUCCESS, payload: data });
  } catch (error) {
   
    dispatch({
      type: CLAIMED_REFER_REWARD_FAIL,
      payload: error.response
        ? error.response.data.error
        : "Unauthorized: please try again after some time",
    });
